...
/**
 * executing after view laid out
 */
inline fun View.doAfterLaidOut(crossinline action: (view: View) -> Unit) {
    if (isLaidOut) {
        action(this)
    } else {
        /**
         * drawToBitmap 需要[View.isLaidOut] 为true；即layout完成之后，
         * [View.layout]方法[View.OnLayoutChangeListener]执行在[View.isLaidOut]设置之前，所以这里[View.post]后执行
         */
        doOnLayout {
            post {
                action(this)
