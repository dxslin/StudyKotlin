 ...
 *      xor                     按位异或
 *      inv                     按位取反
 * 7. 重载复合运算符（+=、-=、*=、/=）：重写plus和plusAssign都可以使用+=，如果两个都重写了且对象是var时
 *      （val会调用plusAssign）使用+=会报错，因此建议只重写一个。如果对象属性是可变的，
 *      建议重写plusAssign（其返回值为Unit），改变对象内容；如果对象属性是不可变的则重写plus
 ...
 *      ++a, a++                inc                 a.inc()
 *      --a, a--                dec                 a.dec()
 * 9. 重载等号运算符（==），只需要重写equals方法（kotlin的===相当于Java的==，kotlin的==相当于Java的equals方法）
 * 10. 重载大小比较运算符（>，<，>=，<=），需要继承并实现 Comparable<T>接口或者增加 compareTo 方法
 *      kotlin提供了compareValues和compareValuesBy一些方法来快捷实现。a >= b 相当于 a.compareTo(b) >= 0
 * 11. 重载下标运算符（[]），对应函数为get和set，如果只是通过下标获取数据实现get方法即可。p[0] = 1 相当于 p.set(0, 1)
 * 12. in运算符，对应函数为contains。point1 in rect2 相当于 rect2.contains(point1)
 ...
 * 15. 解构声明（val (x, y) = Point(1, 3)），对应 componentN 函数，N表示声明中变量的位置，N最大表示5个
 *      比如
 *          val p = Point(1, 3)
 *          val (x, y) = p
 *      就相当于
 ...
 *      如果类定义为了数据类，那么会自动实现componentN函数
 */
