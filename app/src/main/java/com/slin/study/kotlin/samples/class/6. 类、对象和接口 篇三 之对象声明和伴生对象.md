...
##### 三、小结
1. 对象声明时kotlin中定义单例类的方法
2. 伴生对象替代里Java的静态方法和静态字段定义
3. 伴生对象和其他对象一个，可以实现接口，也可以拥有扩展函数和属性
4. 对象表达式是kotlin中针对Java匿名内部类的替代品，并增加了诸如实现多个接口的能力和修改在创建对象的作用域中定义的变量的能力等功能